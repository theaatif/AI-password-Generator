...
const PatternPassword = () => {
  const [pattern, setPattern] = useState("adjective+noun+number+symbol");
  ...
  const generatePassword = () => {
    let pwd = "";
    if (pattern.includes("adjective")) pwd += getRandom(adjectives);
    if (pattern.includes("noun")) pwd += getRandom(nouns);
    if (pattern.includes("number")) pwd += Math.floor(Math.random() * 100);
    if (pattern.includes("symbol")) pwd += getRandom(symbols);
    setPassword(pwd);
  ...
  return (
    <div className="backdrop-blur-lg bg-white/60 border border-white/30 rounded-3xl shadow-2xl p-4 sm:p-8 max-w-md mx-4 sm:mx-auto transition-transform hover:scale-[1.025] hover:shadow-3xl mt-6 mb-8">
      ...
      <div className="mb-4">
        <label className="block mb-1 font-medium text-blue-800">Pattern</label>
        <select
          value={pattern}
          onChange={(e) => setPattern(e.target.value)}
          className="w-full border rounded-xl px-2 py-1 bg-white/80 text-blue-900 text-base"
        >
          <option value="adjective+noun+number+symbol">
            adjective+noun+number+symbol
          </option>
          <option value="adjective+noun+symbol">adjective+noun+symbol</option>
